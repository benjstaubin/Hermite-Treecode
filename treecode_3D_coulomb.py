...
treecode approximation for the screened coulomb potential kernel function. 
During the "compute interaction" stage, multiprocessing is performed by 
spawning c-2 processes, where c is the automatically detected amount of 
physical CPU cores in your system. The tree structure, weights, and data are 
stored to the directory this file is placed in and deleted when the program is 
complete.
...
def ft(array,method):
    ...
    if method == 'lagrange': 
        ...
        z = array[2,:]
        f = np.exp(-k*r)/r
        f_px = (k*r+1)*x*np.exp(-k*r)/r**3
        f_py = (k*r+1)*y*np.exp(-k*r)/r**3
        f_pz = (k*r+1)*z*np.exp(-k*r)/r**3
        f_pyz = ((r*k)**2+3*k*r+3)*y*z*np.exp(-k*r)/r**5
        f_pxz = ((r*k)**2+3*k*r+3)*x*z*np.exp(-k*r)/r**5
        f_pxy = ((r*k)**2+3*k*r+3)*x*y*np.exp(-k*r)/r**5
        f_pxyz = ((r*k)**3+6*(r*k)**2+15*k*r+15)*x*y*z*np.exp(-k*r)/r**7
...
def comp_interaction(target_index, panel, result = 0):
    if panel.numpoints <= 8*p**3:
        return direct_sum(target_index,panel)
    
    dist = np.linalg.norm(points[:,target_index] - panel.center)    
    if panel.radius/dist < MAC: #target and source are well-seperated if so
...
if __name__ == '__main__':
    ...
    #method = 'lagrange' 
    method = 'hermite'
    num_points = N_op[1] #The number of data points    
    p = node_op[6] # the number of interpolating nodes for far-field
    MAC = MAC_op[7] #adjusts ratio of direct sums to far-field approximation    
    n = 1000 #n is the max amount of points per leaf       
         
    ###Random Data Creation (Spherical)
    weights = np.random.uniform(-1,1,num_points)
    a = 0 #lower cube boundary (3D space)
    b = 1 #upper cube boundary (3D space)
    

    #points uniform on sphere
    radius = (a+b)/2 
    ...
    while current_level <= level_count:
        for panel in tree[current_level].values():
            if panel.numpoints <= 8*p**3: continue            
            #source interval
            ax = panel.low[0]
            bx = panel.high[0]
            ay = panel.low[1]
    ...
    ###Saving the tree to file (temporarily) for each instance to use
    tree['MAC'] = MAC #to load into each instance during multi-P
    tree['method'] = method #to load into each instance during multi-P
    tree['node_num'] = p
